...
After ensuring that your experiment generated data of sufficient quality by using **protti**s quality control functions (see [Quality control (QC) workflow](https://jpquast.github.io/protti/articles/quality_control_workflow.html)), you can continue with data analysis. If you just compare a control condition with one treatment condition (or multiple unrelated treatments) you can check out the vignette for the analysis of binary data. For experiments with a dose response treatment we can fit sigmoidal log-logistic regression curves to the data and determine significantly changing precursors, peptides or proteins. 

An experiment with a dose response treatment usually contains multiple conditions that are treatments with e.g. a drug in increasing concentrations. Commonly, the first concentration is 0 (thus just the treatment with the solvent of the drug). Furthermore, dose response treatments require a minimal number of treatments to fit curves with sufficient quality. In **protti** at least 5 conditions should be present. Another consideration is the range of concentrations. On the one hand they should not be too close together since effects are usually best identified over a larger concentration range. On the other hand you should make sure to not space them out too far, however that is usually less of a problem. It is generally advised to space them evenly on a logarithmic scale of the base 10 or $e$ (Euler's number). You can also include steps in between e.g. 100, 500, 1000 or 100, 200, 1000 for a log10 scale. Using a rather broad concentration range for an experiment in which you do not know what to expect is generally advisable. 

**protti** is fitting four-parameter log-logistic dose-response models to your data. It utilizes the `drm()` and `LL.4()` functions from the [`drc`](http://journals.plos.org/plosone/article?id=10.1371/journal.pone.0146021) package for this. If you chose to, you can also select a non logarithmic model using the `L.4()` function in case your data does not have a log-logistic but a logistic distribution. 

For limited proteolysis coupled to mass spectrometry ([LiP-MS](https://www.nature.com/articles/nbt.2999)) data, dose-response curves have been used previously for the identification of drug binding sites in complex proteomes ([Piazza 2020](https://www.nature.com/articles/s41467-020-18071-x)). Since LiP-MS data is usually analysed on the peptide or precursor level, utilizing additional information about peptide behavior from multiple conditions reduces false discovery rate. 

# Getting started

Before starting the analysis of your data you need to load **protti** and additional packages utilized for the analysis. As described on the [main page](https://jpquast.github.io/protti/index.html) of **protti**, the [tidyverse](https://www.tidyverse.org) package collection is by design working well together with the rest of the package. You can load packages after you installed them with the `library()` function.
...
For this vignette we are using a subset of proteins from a rapamycin dose response treatment of HeLa cell lysates followed by LiP-MS. Rapamycin forms a gain-of-function complex with the FK506-binding protein (FKBP12) that binds and allosterically inhibits mTORC1 ([Sabatini 1994](https://www.sciencedirect.com/science/article/pii/0092867494905703?via%3Dihub)). Since Rapamycin is known to be a highly specific drug, we expect FKBP12 to show up as one of the only interacting proteins. 

We included 39 random proteins and FKBP12 in this sample dataset. The proteins were sampled using the seed 123. 
...
In this example the rapamycin dose-response dataset is included in protti and you can easily use it by calling the `data()` function. This will load the data into your R session.
...
In addition, to the removal of decoys we also remove any non-proteotypic peptides (`pep_is_proteotypic`) from our analysis. This is specific for the analysis of LiP-MS data. If a peptide is non-proteotypic it is part of two proteins. If we detect a change in non-proteotypic peptides it is not possible to clearly assign which of the two proteins it comes from and which is the protein affected by your treatment. 
...
```{r intensity_distribution, fig.width = 8, fig.height = 6}
...
Before we fit dose-response models to our data, we can check how samples cluster. Ideally, samples that belong to the same treatment concentration cluster together. A common way that you can analyse this is correlation based hirachical clustering using the `qc_sample_correlation()` function from **protti**. Basically, the correlation of samples is calculated based on the correlation of precursor (or peptide/protein) intensities. By default `qc_sample_correlation()` uses spearman correlation, however you can change that to any correlation supported by the `cor()` function. 

Note: Many of **protti**s plotting function also have the option to display an interactive version of the plot with an `interactive` argument.

```{r sample_correlation, fig.width = 8, fig.height = 6}
...
For our Rapamycin dataset we cannot identify any clear sample clustering based on correlation, because only a small subset of proteins was selected for this example. Furthermore, datasets in which only a few changes are expected commonly do not cluster nicely. This is because precursors are having the same intensity values throughout all conditions and clustering is in this case based on variance. If many changes occur, samples should usually cluster nicely. 
...
If you have to fit many models this function may take about an hour or longer. If you have the computational resources you can speed this up by using `parallel_fit_drc_4p()`, a parallel version of this function. 

Note: Keep in mind that by spreading the model fitting over multiple cores, more memory than usual is needed. Furthermore, using the maximal number of available cores may not be the most efficient, because it takes time to copy the data too and from each core, which will not be performed in parallel. Therefore, it is adviced that the run time on each core is at least as long as the time to set up (data transfer, environment preparation etc.) all cores.
...
If we have a look at all precursors based on their rank (calculated from correlation and ANOVA q-value), we can see that as expected most of them are FKBP12 (P62942) peptides. One benefit of analysing LiP-MS data on the precursor level is that multiple lines of evidence for a change in the specific peptide can be used. Therefore, it is always good to check if there are other precursors of a good scoring peptide that do not show any regulation at all. This would mean that the reason for the observed response is not based on a biological effect.
...
```{r true_positive_rate, echo=FALSE, fig.width = 8, fig.height = 6}
fit %>% 
...
```
It may happen that many precursors pass the filtering threshold and are thus potentially affected by the treatment. This is because filtering threshold are not very strict. If you have information about real targets of your treatment, you can set a manual rank cutoff based on the true positive rate evaluation. It is likely that precursors among known targets are also regulated by your treatment. Precursors that have a low rank with not many true positive hits in their vicinity might not be actual hits. You can set a manual rank cutoff at a point where the true positive rate flattens. 

If the true positive rate does not drop and almost all of your hits are true positives, you can try to reduce the correlation cutoff in `fit_drc_4p()` to be even less conservative. This might allow you to find novel interaction partners. If the above true positive rate evaluation would have been observed for the complete Rapamycin dataset, such a step might have been a good practice. However, in the complete dataset a lot more precursors pass the filtering and the true positive rate drops also there after about the first 25 ranks. This would mean, that many of the additionally ranked precursors could be false positive interactions. 

## Model plotting

After model fitting, you can easily visualise model fits by plotting with the `plot_drc_4p()` function. You can provide one or more precursor IDs to the `targets` argument or just return plots for all precursors with `targets = "all"`. Make sure that the amount of plots is reasonable to return within R. You can also directly export your plots by setting `export = TRUE`. 
...
```{r model_plot, fig.width = 8, fig.height = 6}
# Model plotting
plot_drc_4p(fit,
            grouping = eg_precursor_id,
            dose = r_condition,
...
Datasets are often tested for gene ontology enrichments (`go_enrichment()`). Alternatively, the KEGG database can provide pathway annotations that can be used to test for pathway enrichments (`kegg_enrichment()`). 

If you know which proteins bind or interact with your specific treatment, you can provide your own list of true positive hits and check if these are enriched in your significant hits by using **protti**s `treatment_enrichment()` function. For our LiP-MS experiment using Rapamycin we are probing direct interaction with proteins in contrast to functional effects. The only protein that Rapamycin is known to bind to is FKBP12 and testing the significance of enrichment for a single or even a few proteins does not make much sense. However, testing for enrichment is especially useful if your treatment interacts with many proteins, since it can help you to reduce the complexity of your result.
...
As mentioned earlier, for this specific dataset enrichment and also network analysis does not make a lot of sense. This is mainly because we only use a very small random subset of the complete dataset. Furthermore, Rapamycin is known to only interact with FKBP12 and enrichment analysis will not properly work if your treatment only interacts with a few protein. 
...
# In this case HeLa cells were used to the organism of interest is homo sapiens (hsa)
