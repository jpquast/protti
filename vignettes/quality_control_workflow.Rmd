...
Before we can start analysing our data, we need to load the **protti** package. This is done by using the base R function `library()`. In addition, we are also loading the packages `magrittr` and `dplyr`. Both `magrittr` and `dplyr` are part of the [tidyverse](https://tidyverse.org/), a collection of packages that provide useful functionalities for data processing and visualisation. If you use many tidyverse packages in your workflow you can easily load all at once by calling `library(tidyverse)`.
...
If you want to control how names (e.g. samples) are displayed on the x-axis, you can convert the corresponding column in your data frame to a `factor`. Factors have an inherent order of levels, which will be used when creating plots.

This means that you can define **any order you like**, not just alphabetical or numerical order.
For example, you might want to order samples by condition, time point, or replicate:

```{r reorder_samples, eval = FALSE}
data_reordered <- data %>% 
  # mutate() modifies the sample column
  # the levels argument defines the order in which samples will appear in plots
  mutate(sample = factor(sample, levels = c("sample_3", "sample_1", "sample_2", "sample_4", "sample_6", "sample_5")))
```

This can be useful if you want to highlight a specific order in your experiment (e.g. time course or treatment groups).
...
Run intensities can also be assessed by plotting the median run intensities as a line plot. This helps you quickly assess if there are any trends in your data. The function to use for this analysis is `qc_median_intensities()`. 
