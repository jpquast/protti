...
For LiP-MS analysis we commonly remove non-proteotypic peptides (i.e. peptides that could come from more than one protein). If you detect a change in non-proteotypic peptides it is not possible to clearly assign which of the two proteins it comes from and which is the protein affected by your treatment. If you are using the output from Spectronaut you will find a column called "pep_is_proteotypic". This column contains logicals indicating whether your peptide is proteotypic or not. 
...
```{r coverage_plot, fig.align= "center", fig.width = 6, fig.height = 5}
...
Next we are going to visualise the output of the previously performed hypothesis test to assess the results of our experiment. For this we are going to plot a volcano plot with fold-changes on the x-axis and the adjusted p-value (q-value) on the y-axis. 
...
To see how the individual precursors in our target protein are changing with the treatment we are plotting profile plots by using the function `plot_peptide_profiles()`.

```{r protile_plot, fig.align = "center", fig.width = 20, fig.height = 6, message = FALSE, warning = FALSE}
...
diff_abundance_significant <- diff_abundance %>%
