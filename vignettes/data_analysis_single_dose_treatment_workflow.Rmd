...
By adjusting our p-values for multiple testing we are ensuring that we keep the false discovery rate low. P-values are corrected for multiple testing using the Benjamini-Hochberg (FDR) correction.
...
_Note: If very little observations are changing significantly or if the p-values are not very low to begin with it can happen that only a few significant hits remain after adjusting the p-value._
