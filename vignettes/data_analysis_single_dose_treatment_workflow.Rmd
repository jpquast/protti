...
_Note: The use of the `filter_cv()` function is optional. It might remove a lot of your data if your experiment was noisy. However, especially in these cases, the function will remove peptides with poor quality and should improve the result. It is very important that if you use this function you should not use the moderated t-test or proDA algorithm on your data for differential abundance estimation and significance testing. This likely will lead to an inflated false positive rate because it alters the distributional assumptions of these tests ([Bourgon 2010](https://www.pnas.org/content/107/21/9546))._
...
For this subset of data the distribution of p-values is relatively flat and there is no large increase in values in the low p-value range. This is likely because, especially for certain LiP-MS experiments, only a very small fraction of peptides show changes. If so, it can happen that none or only a few significant hits remain after adjusting the p-value using the Benjamini-Hochberg correction. The Benjamini-Hochberg adjustment is an appropriate method to use in such a case, however, there might be correction methods that better account for the low number of true positive hits.

It is recommended to always check the p-value distribution after the statistical test in order to make sure that the mathematical assumptions for multiple testing correction are met. Any histogram that does not have a uniform distribution (that is not flat) with or without a specific enrichment for very low p-values (due to a treatment induced effect) might indicate a failed mathematical assumption that is caused by correlation across proteins or peptides, correlation across samples or unobserved confounders ([Efron 2010, chapter 6](https://www.cambridge.org/core/services/aop-cambridge-core/content/view/0BD692B5CFF1C51B73D480B72B0B3A28/9780511761362c6_p89-112_CBO.pdf/theoretical_permutation_and_empirical_null_distributions.pdf)). In this case you should carefully evaluate where the correlation comes from and see if you can correct for it with a method such as local FDR correction ([Efron 2010, chapter 5](https://www.cambridge.org/core/services/aop-cambridge-core/content/view/010A779FBAA2456833678F25F7BDBB2B/9780511761362c5_p70-88_CBO.pdf/local_false_discovery_rates.pdf)).
