...
It is very important, that each unit of the level you perform your analysis on (e.g. peptide) has a single unique intensity associated with it. If e.g. a peptide would have two different intensities, **protti** would not know how to deal with this and many functions will likely fail. 
...
protein_groups_filtered <- protein_groups %>%
  mutate(is_only_identified_by_site = ifelse(only_identified_by_site == "+", TRUE, FALSE)) %>%
  filter(is_only_identified_by_site == FALSE) %>% # remove proteins only identified by PTM
  mutate(is_potential_contaminant = ifelse(potential_contaminant == "+", TRUE, FALSE)) %>%
  filter(is_potential_contaminant == FALSE) %>% # remove potential contaminants
  mutate(is_reverse = ifelse(reverse == "+", TRUE, FALSE)) %>%
  filter(is_reverse == FALSE) %>% # remove reverse hits
  filter(peptides > 2) # we filter out proteins that have less than 3 identified peptides

# Change wide format to long format and create new columns called `r_file_name`and `intensity`
protein_groups_long <- protein_groups_filtered %>%
  pivot_longer(
