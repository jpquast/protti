...
#' @param pipeline optional, the name of the column containing information about the pipline used. Eg. "LiP" or "tryptic control".
...
#' )
#' }
qc_sample_correlation <- function(data, sample, grouping, intensity, condition, pipeline = NULL, run_order = NULL, method = "spearman", interactive = TRUE){
  correlation <- data %>%
  ...
  annotation <- data %>%
    dplyr::distinct({{sample}}, {{condition}}, {{pipeline}}, {{run_order}}) %>%
  ...
  hirachical_clustering <- stats::hclust(distance)
  dendrogram <- stats::as.dendrogram(hirachical_clustering)
