...
#' @param grouping the name of the column containing peptide, precursor or protein information depending on which level data is evaluated.
...
#'
#' @return A data frame that contains the reference condition paired with each treatment condition. The \code{conditions} column contains the name of the treatment for the specific reference/treatment pair. The \code{missingness} column reports the type of missingness. 
#' \itemize{
...
data <-  data %>%
    ...
    dplyr::ungroup() %>%
    dplyr::mutate(conditions = ifelse(
      {{condition}} %in% unique(pull(data, !!ensym(condition)))[!unique(pull(data, !!ensym(condition))) %in% ref_condition],
      {{condition}},
      list(unique(pull(data, !!ensym(condition)))[!unique(pull(data, !!ensym(condition))) %in% ref_condition])
    ))%>%
    tidyr::unnest(.data$conditions) 
  
data %>%
    dplyr::distinct({{condition}}, {{grouping}}, .data$n_detect, .data$n_replicates, .data$conditions) %>%
    dplyr::group_by({{grouping}}, .data$conditions) %>%
  ...
  dplyr::select(-.data$n_detect, -.data$n_replicates) %>%
   dplyr::left_join(data, by = c("conditions", rlang::as_name(enquo(condition)), rlang::as_name(enquo(grouping)))) %>%
