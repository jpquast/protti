...
correct_lip_for_abundance = function(
  ...
  corrected_data = combined_data %>%
    ...
    dplyr::mutate(df = .data$numer/.data$denom) %>%
    dplyr::mutate(tval = .data$adj_diff / .data$adj_std_error) %>%
    dplyr::mutate(pval = 2*stats::pt(abs(.data$tval), .data$df, lower.tail = FALSE))
  ...
  adjusted_data = dplyr::left_join(x = corrected_data,
    ...
    dplyr::select(- .data$numer, - .data$denom, - .data$tval)
