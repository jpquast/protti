#' Fetch information about structure protein-metal binding from MetalPDB
#'
...
#' @param representative optional character, if only information of representative sites of a family should be retrieved it can 
#' be specified here. This can only be "true" if specified. A representative site is a site selected to represent a cluster of equivalent sites. The 
...
#'
#' @return A data frame that contains information about protein-metal binding sites.
#' @import dplyr
#' @import progress
#' @import purrr
#' @import tidyr
...
fetch_metal_pdb <- function(id_type = "uniprot",
  ...
  if (!missing(site_type)) {
    if (length(site_type) > 1) {
      stop('Please only provide one value for the "Site Type (Nuclearity)"')
    }
  ...
  if (!missing(pfam)) {
    if (length(pfam) > 1) {
      stop('Please only provide one value for the "Pfam domain name"')
    }
  ...
  if (!missing(cath)) {
    if (length(cath) > 1) {
      stop('Please only provide one value for the "Cath ID"')
    }
  ...
  if (!missing(scop)) {
    if (length(scop) > 1) {
      stop('Please only provide one value for the "SCOP ID"')
    }
  ...
  if (!missing(representative)) {
    if (representative != "true") {
      stop('Please only provide "true" for the "representative" argument')
    }
  ...
  if (!missing(metal)) {
    if (length(metal) > 1) {
      stop('Please only provide one value for the "Metal symbol"')
    }
  ...
  if (!missing(ligands)) {
    if (length(ligands) > 1) {
      stop('Please only provide one value for the "Type of metal ligand residue"')
    }
  ...
  if (!missing(geometry)) {
    if (length(geometry) > 1) {
      stop('Please only provide one value for the "Metal geometry"')
    }
  ...
  if (!missing(coordination)) {
    if (length(coordination) > 1) {
      stop('Please only provide one value for the "Metal coordination number"')
    }
  ...
  if (!missing(donors)) {
    if (length(donors) > 1) {
      stop('Please only provide one value for the "Type of donor atom of metal ligand residues"')
    }
  ...
  content_ligand <- content_metal %>%
    ...
    dplyr::rename(
      residue_pdb_number_metal = .data$residue_pdb_number,
      atom_pdb_number_metal = .data$atom_pdb_number,
