...
#' @param show_progress a logical value that controls whether progress bars are shown.
#' If `FALSE` (default), progress bars are displayed during model fitting,
#' correlation calculation, p-value calculation and prediction steps.
#' If `TRUE`, all progress bars are suppressed. This does not affect warnings
#' or messages generated by the function.
...
fit_drc_4p <- function(data,
  ...
  # fit models
  progress_option_model_fit <- if (show_progress) {
    list(
      type = "iterator",
      format = "1/4 Model fitting {cli::pb_bar} {cli::pb_percent} ({cli::pb_current}/{cli::pb_total}) ETA: {cli::pb_eta}",
      clear = TRUE
    )
  } else {
    FALSE
  ...
  fit_objects <- purrr::map(
    ...
    .progress = progress_option_model_fit
  )

  # extract information from fit objects and calculate correlation

  progress_option_correlation <- if (show_progress) {
    list(
      type = "iterator",
      format = "2/4 Calculating correlations {cli::pb_bar} {cli::pb_percent} ({cli::pb_current}/{cli::pb_total}) ETA: {cli::pb_eta}",
      clear = TRUE
    )
  } else {
    FALSE
  ...
  correlation_output <- input %>%
    purrr::map2(
      ...
      .progress = progress_option_correlation
    ) %>%
  ...
  progress_option_pvalues <- if (show_progress) {
    list(
      type = "iterator",
      format = "3/4 Calculating p-values {cli::pb_bar} {cli::pb_percent} ({cli::pb_current}/{cli::pb_total}) ETA: {cli::pb_eta}",
      clear = TRUE
    )
  } else {
    FALSE
  ...
  p_value_correlation <- input %>%
    purrr::map2(
      ...
      .progress = progress_option_pvalues
    ) %>%
  ...
  progress_option_predict <- if (show_progress) {
    list(
      type = "iterator",
      format = "4/4 Predicting curves for plots {cli::pb_bar} {cli::pb_percent} ({cli::pb_current}/{cli::pb_total}) ETA: {cli::pb_eta}",
      clear = TRUE
    )
  } else {
    FALSE
  ...
  predictions <- fit_objects %>%
    purrr::map2(
      ...
      .progress = progress_option_predict
    ) %>%
