...
find_drc_missingness <- function(data,
  ...
  if (!missing(max_dose_replicates)) {
    dose_replicate_data <- tibble::tibble(
      max_replicates = max_dose_replicates,
      {{ dose }} := complete_doses
    )
  }
  ...
  clusters <- cluster_prep %>%
    ...
    tidyr::pivot_wider(
      values_from = c(.data$n_0, .data$mean, .data$max_mean, .data$detected, .data$undetected),
  ...
  if (nrow(test_data_prep) != 0) {
    test_data <- test_data_prep %>%
      dplyr::rowwise() %>%
      dplyr::mutate(pval = fisher.test(matrix(c(
        .data$detected_cluster1, .data$detected_cluster2,
        .data$undetected_cluster1, .data$undetected_cluster2
      ), nrow = 2))$p.value) %>%
      dplyr::ungroup() %>%
  ...
  result <- test_data %>%
    dplyr::rowwise() %>%
    dplyr::mutate(
      incomplete_mean = min(.data$mean_cluster1, .data$mean_cluster2),
    ...
    dplyr::ungroup() %>%
    dplyr::mutate(
      incomplete_cluster_size = ifelse(.data$incomplete_mean == .data$mean_cluster1,
        .data$cluster_split_id,
  ...
  result_for_plot <- cluster_prep_subset %>%
    dplyr::left_join(cluster_subset, by = c(
      rlang::as_name(rlang::enquo(grouping)),
      "cluster_split_id"
    )) %>%
